...
export default class ServerLinks {
    public static revDanbooru = async (bytes: number[]) => {
        ...
        const buffer = await fetch(original, {headers: {"user-agent": `user #${process.env.DANBOORU_USER}`}}).then((r) => r.arrayBuffer())
    ...
    public static booruLinks = async (bytes: number[]) => {
        ...
        const appendExtraLinks = async (link: string) => {
            try {
                const post = await serverFunctions.http.fetch(`${link}.json`).then((r) => r.json()) as any
                const mediaId = post.media_asset.id
                const html = await serverFunctions.http.fetch(`https://danbooru.donmai.us/media_assets/${mediaId}`).then((r) => r.text())
    ...
    public static testBooruLinks = async (booruLinks: string[], rating: PostRating) => {
        ...
        if (danbooruLink) {
            try {
            const json = await serverFunctions.http.fetch(`${danbooruLink}.json`).then((r) => r.json()) as any
        ...
        if (!Object.keys(tagData).length && gelbooruLink) {
            ...
            const json = await serverFunctions.http.fetch(url).then((r) => r.json()) as any
