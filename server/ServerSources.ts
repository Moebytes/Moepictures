...
export default class ServerSources {
    ...
    public static pixivLookup = async (pixivLink: string, rating: PostRating) => {
        ...
        try {
            const result = await serverFunctions.http.proxyFetch(`https://danbooru.donmai.us/posts.json${process.env.DANBOORU_API_KEY}&tags=pixiv_id%3A${pixivID}`).then((r) => r.json()) as any
            if (result.length) {
    ...
    public static twitterLookup = async (twitterLink: string, rating: PostRating) => {
        ...
        for (const redirect of redirects) {
            ...
            try {
                ...
                const result = await serverFunctions.http.proxyFetch(`https://danbooru.donmai.us/posts.json${process.env.DANBOORU_API_KEY}&tags=source%3A${cleaned}`).then((r) => r.json()) as any
                if (result.length) {
                    if (result[0].rating === "q") rating = functions.highestRating(rating, functions.r17())
                    if (result[0].rating === "e") rating = functions.highestRating(rating, functions.r18())
                }
    ...
    public static danbooruLookup = async (danbooruLink: string, rating: PostRating) => {
        ...
        let danbooruPost = await serverFunctions.http.proxyFetch(`https://danbooru.donmai.us/posts/${id}.json${process.env.DANBOORU_API_KEY}`).then((r) => r.json()) as any
        ...
        let commentaries = await serverFunctions.http.proxyFetch(`https://danbooru.donmai.us/artist_commentaries.json${process.env.DANBOORU_API_KEY}&commit=Search&search[post_id]=${id}`).then((r) => r.json()) as any
        if (commentaries[0]) {
