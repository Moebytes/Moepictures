...
const DeletedPostRow: React.FunctionComponent<Props> = (props) => {
    ...
    const deletePost = async () => {
        await functions.http.delete("/api/post/delete", {postID: props.post.postID}, session, setSessionFlag)
        props.onDelete?.()
    }
    ...
    const imgClick = (event: React.MouseEvent) => {
        functions.post.openPost(props.post, event, navigate, session, setSessionFlag)
    }
            ...
            <div className="historyrow-container-row">
                <div className="historyrow-container">
                    <div className="historyrow-user-container" onMouseEnter={() => setEnableDrag(false)} onMouseLeave={() => setEnableDrag(true)}>
                        <span className="historyrow-user-text" style={{color: "var(--deletedColor)"}}>{i18n.time.deleted} {functions.date.timeUntil(props.post.deletionDate, i18n)}</span>
                        ...
                        <span className="historyrow-text"><span className="historyrow-label-text-strong">{i18n.sort.posted}: </span>{props.post.posted ? functions.date.formatDate(new Date(props.post.posted)) : i18n.labels.unknown}</span>
                        ...
                        <span className="historyrow-text"><span className="historyrow-label-text-strong">{i18n.labels.source}: </span><span className="historyrow-label-link" onClick={() => window.open(props.post.source, "_blank")}>{functions.util.getSiteName(props.post.source, i18n)}</span></span>
