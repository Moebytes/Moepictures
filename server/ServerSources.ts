...
export default class ServerSources {
    ...
    public static pixivLookup = async (pixivLink: string, rating: PostRating) => {
        ...
        try {
            const result = await functions.http.proxyFetch(`https://danbooru.donmai.us/posts.json?tags=pixiv_id%3A${pixivID}`).then((r) => r.json()) as any
            if (result.length) {
    ...
    public static twitterLookup = async (twitterLink: string, rating: PostRating) => {
        ...
        for (const redirect of redirects) {
            ...
            try {
                ...
                const result = await functions.http.proxyFetch(`https://danbooru.donmai.us/posts.json?tags=source%3A${cleaned}`).then((r) => r.json()) as any
                if (result.length) {
                    if (result[0].rating === "q") rating = functions.highestRating(rating, functions.r17())
                    if (result[0].rating === "e") rating = functions.highestRating(rating, functions.r18())
                }
    ...
    public static danbooruLookup = async (danbooruLink: string, rating: PostRating) => {
        ...
        let danbooruPost = await functions.http.proxyFetch(`https://danbooru.donmai.us/posts/${id}.json`).then((r) => r.json()) as any
        ...
        let commentaries = await functions.http.proxyFetch(`https://danbooru.donmai.us/artist_commentaries.json?commit=Search&search[post_id]=${id}`).then((r) => r.json()) as any
        if (commentaries[0]) {
